...
- [x] Verifikasi visual lintas-breakpoint (Playwright) semua halaman M3 — **SELESAI** (commit `5c30bde`, **S/Deputi Sonet**, review+gate **C/Opus**): Playwright chromium + auth temp-user + 28 baseline (7 hal × {375,768,1024,1440}), 29/29 pass. **TEMUAN**: overflow ≤375px di /sales/pos/settings — akar tunggal shell Sidebar `position:fixed` tanpa `@media` collapse (drawer mobile ditunda sejak M3); 768px+ bersih. → **KEPUTUSAN Master: lebar HP <375px BUKAN target → WON'T-FIX (skip).** Lantai dukungan web = **768px+** (tablet/desktop). Kasir-mobile akan ditangani lewat `.apk` terpisah ([[cashier-mobile-app-goal]]), bukan web responsif. Drawer responsif TIDAK dijadwalkan.
